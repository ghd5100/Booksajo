...
//					//System.out.println("class_nm:" + objj23.get("class_nm")); // 이거는 디테일 페이지에서 쓸수있겠다.
//					
//					
//					
...
*/
		...
		/*	
			...
			for (int i = 0; i < class_nolist.size(); i++) {
				...
				// 소 카테고리 추출
				String b = class_nolist.get(i).substring(1);
...
/*			
			 for (String string : class_s_list) { 
...
/*
			...
			for (int j = 0; j < isbnlist.size(); j++) {
				...
				http: // data4library.kr/api/loanItemSrch?authKey=[발급받은키]&gender=0&from_age=6&to_age=10&region=11;22&addCode=0&kdc=6&pageNo=1&pageSize=10

				serviceKey4 = "http://data4library.kr/api/loanItemSrch?authKey=bd1ca3ab552dd10a147c1e0594dd11525c60dcdb8f5f22d0cd940b3faa2f3c69";
				String urlStr2 = serviceKey4 + "&gender=0&from_age=20&to_age=50&region=11;22&addCode=0&kdc="
						+4+ "&format=json";
				...
				// 이걸 포문을 돌려주면 꺼내서 보면 다 꺼내쥠!
				JSONObject recbooks = (JSONObject) json12.get("book");
				System.out.println("class_nm뽑음:" + recbooks.get("class_nm"));
				Object classNm = recbooks.get("class_nm");
			...
			// 3단계로 나뉘는데.. 어떻게 디비랑 맞춰야하려나..
			// 이 3개 카테고리를 다 뷰에 보여줘야하는데 디비에서 매칭해서,
			// 스플릿해서 ..다 스트링 배열로 만든다음에.
			// 보여주는건 끝에 일자리만 가져가도 될것같은데..

			// 스플릿에 또 스플릿 해야겠군.
			// ,없으면 스플릿하지않는 조건. 그냥넣어주고 ,있으면 스플릿해서 나눠서 배열에 넣어줘서 보내줘야겠따.
*/
		} catch (
